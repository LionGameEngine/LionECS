...
public struct Requires3ComponentsQuery<ComponentManagers: PComponentManagers, R0: PComponent, R1: PComponent, R2: PComponent>: PEntityQuery {
    public typealias Result = EntityQueryResult<(R0, R1, R2)>
    ...
    public func resolveWith(requester: EntityRequester<ComponentManagers>) throws -> Result {
        ...
        for entity in try getFilteredEntities(requester: requester) {
            result[entity] = (r0Components[entity]!, r1Components[entity]!, r2Components[entity]!)
        }
        return Result.init(comp: result)
    }
    
    public func getFilters() -> [PEntityFilter] {
        return [Requires<R0>(), Requires<R1>(), Requires<R2>()] + exclusionFilters
    }
}
