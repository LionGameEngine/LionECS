//
//  Requires5ComponentsQuery.swift
//  ECS
//
...
public struct Requires5ComponentsQuery<ComponentManagers: PComponentManagers, R0: PComponent, R1: PComponent, R2: PComponent, R3: PComponent, R4: PComponent>: PEntityQuery {
    public typealias Result = EntityQueryResult<(R0, R1, R2, R3, R4)>
    ...
    public func resolveWith(requester: EntityRequester<ComponentManagers>) throws -> Result {
        ...
        for entity in try getFilteredEntities(requester: requester) {
            result[entity] = (r0Components[entity]!, r1Components[entity]!, r2Components[entity]!, r3Components[entity]!, r4Components[entity]!)
        }
        return Result.init(comp: result)
    }
    
    public func getFilters() -> [PEntityFilter] {
        return [Requires<R0>(), Requires<R1>(), Requires<R2>(), Requires<R3>(), Requires<R4>()] + exclusionFilters
    }
}
